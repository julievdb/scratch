...
function _themename_content_width() {
   ...
   if(is_single() && $post->post_type === 'post') {
      $layout = get_post_meta( $post->ID, '__themename_post_layout', 'full' );
...
function _themename_image_sizes( $sizes, $size, $image_src, $image_meta, $attachment_id ) {
   ...
   if($content_width <= $width) {
      if($layout === 'full') {
         ...
         $sizes = '(max-width: 640px) calc(100vw - 1.25rem * 2 - 0.625rem * 2 - 2px), (max-width: 1200px) calc(100vw - 33.33333vw - 0.625rem *  - 1.25rem * 2 - 2px), ' . $content_width . 'px';
      }
   } else {
